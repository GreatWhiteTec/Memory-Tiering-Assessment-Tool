...
# ==========================================
# --- MAIN UI SETUP (Dual-Grid) ---
# ==========================================
$form = New-Object Windows.Forms.Form
$form.Text = "VMware Memory Tiering Assessment Tool"
...
$lblNote = New-Object Windows.Forms.Label; $lblNote.Text = "*Selectable ONLY if Memory Tiering is enabled"; $lblNote.Location = New-Object Drawing.Point(320, 45); $lblNote.AutoSize = $true; $lblNote.ForeColor = [System.Drawing.Color]::DimGray; $lblNote.Font = New-Object Drawing.Font("Segoe UI", 8, [Drawing.FontStyle]::Italic); $form.Controls.Add($lblNote)
...
# NEW UI: Host Grid
$lblHostGrid = New-Object Windows.Forms.Label; $lblHostGrid.Text = "Host Breakdown (DRAM & Active Load):"; $lblHostGrid.Location = New-Object Drawing.Point(20, 205); $lblHostGrid.AutoSize = $true; $lblHostGrid.Font = New-Object Drawing.Font("Segoe UI", 9, [Drawing.FontStyle]::Bold); $form.Controls.Add($lblHostGrid)
$dgHosts = New-Object Windows.Forms.DataGridView; $dgHosts.Location = New-Object Drawing.Point(20, 225); $dgHosts.Size = New-Object Drawing.Size(780, 160); $dgHosts.AutoSizeColumnsMode = "Fill"; $dgHosts.ReadOnly = $true; $form.Controls.Add($dgHosts)

# NEW UI: VM Grid
$lblVMGrid = New-Object Windows.Forms.Label; $lblVMGrid.Text = "Top VM Consumers:"; $lblVMGrid.Location = New-Object Drawing.Point(20, 395); $lblVMGrid.AutoSize = $true; $lblVMGrid.Font = New-Object Drawing.Font("Segoe UI", 9, [Drawing.FontStyle]::Bold); $form.Controls.Add($lblVMGrid)
$dgVMs = New-Object Windows.Forms.DataGridView; $dgVMs.Location = New-Object Drawing.Point(20, 415); $dgVMs.Size = New-Object Drawing.Size(780, 275); $dgVMs.AutoSizeColumnsMode = "Fill"; $dgVMs.ReadOnly = $true; $form.Controls.Add($dgVMs)
...
$btnRun.Add_Click({
    ...
    try {
        ...
        # Pre-fetch all powered-on VMs to drastically speed up processing
        $allVMs = Get-VM -Location $clusterName | Where-Object {$_.PowerState -eq "PoweredOn"}
        ...
        $script:hostResults = New-Object System.Collections.Generic.List[PSObject]
        $script:vmResults = New-Object System.Collections.Generic.List[PSObject]
        ...
        foreach ($h in $hosts) {
            ...
            if ($hostIsTiered) { $hostDramGB = $rawTotalGB / $divisor } else { $hostDramGB = $rawTotalGB }
            $totalDramGB_Sum += $hostDramGB
            
            # Process VMs specific to this host
            $hostVMs = $allVMs | Where-Object { $_.VMHost.Name -eq $h.Name }
            $hostActiveGB = 0
            ...
            foreach ($vm in $hostVMs) {
                ...
                $script:vmResults.Add([PSCustomObject]@{
                    Host           = $h.Name
                    VMName         = $vm.Name
            ...
            $hostPct = if ($hostDramGB -gt 0) { [math]::Round(($hostActiveGB / $hostDramGB) * 100, 2) } else { 0 }
            
            $script:hostResults.Add([PSCustomObject]@{
                HostName   = $h.Name
                Tiering    = if ($hostIsTiered) { "Enabled" } else { "Disabled" }
                Total_RAM  = [math]::Round($rawTotalGB, 0)
                DRAM_Tier  = [math]::Round($hostDramGB, 0)
                Active_GB  = $hostActiveGB
                Active_Pct = $hostPct
            })
        }
        ...
        $script:sortedVMResults = $script:vmResults | Sort-Object Active_Pct -Descending
        ...
        # Update Grid UIs
        $dgHosts.DataSource = [System.Collections.ArrayList]$script:hostResults
        $dgVMs.DataSource = [System.Collections.ArrayList]$script:sortedVMResults
        
        $pb.Value = [math]::Min([int]$ratio, 100)
        $label.Text = "Aggregate Cluster DRAM: $finalDramGB GB | Active Workload: $totalActiveGB GB ($ratio%) | Status: $statusText"
...
$btnExport.Add_Click({
    ...
    $save.FileName = "vSphere9_MemoryTier_VMs.csv"
    if ($save.ShowDialog() -eq "OK") {
        
        # Export VM Data
        $script:sortedVMResults | Export-Csv -Path $save.FileName -NoTypeInformation
        
        # Smartly generate a second filename for the Host Data and Export it
        $hostFile = $save.FileName -replace "VMs.csv", "Hosts.csv"
        if ($hostFile -eq $save.FileName) { 
            # Fallback if they manually renamed the file and removed 'VMs'
            $hostFile = $save.FileName -replace "\.csv$", "_Hosts.csv" 
        }
        $script:hostResults | Export-Csv -Path $hostFile -NoTypeInformation
        
        [System.Windows.Forms.MessageBox]::Show("Export successful!`n`nSaved two files to your directory:`n1. $(Split-Path $save.FileName -Leaf)`n2. $(Split-Path $hostFile -Leaf)", "Export Complete")
    }
})
